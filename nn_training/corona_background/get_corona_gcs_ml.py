...
def get_corona(sat, imsize=None, diff=True, rnd_rot=False):
    ...
    size_occ=[1.4, 1.4, 2]# Occulters size for [sat1, sat2 ,sat3] in [Rsun] 3.7
    ...
    # lasco
        ...
        # Header fix
        h0['OBSRVTRY'] = 'SOHO'
        coordL2 = get_horizons_coord(-21, datetime.datetime.strptime(h0['DATE-OBS'], "%Y-%m-%dT%H:%M:%S.%f"), 'id')
        #coordL2ston = get_body_heliographic_stonyhurst('-21', time=datetime.datetime.strptime(h0['DATE-OBS'], "%Y-%m-%dT%H:%M:%S.%f"))
        ...
        h0['CRLT_OBS'] = coordL2carr.lat.deg
        h0['CRLN_OBS'] = coordL2carr.lon.deg
        h0['HGLT_OBS'] = coordL2ston.lat.deg
        h0['HGLN_OBS'] = coordL2ston.lon.deg
    else:
