...
OPATH = '/gehme-gpu/projects/2020_gcs_with_ml/data/gcs_ml_1VP_100k' #'/gehme/projects/2020_gcs_with_ml/data/forwardGCS_test'
...
df = pd.DataFrame(np.column_stack(all_par), columns=par_names)
mask_prev = None
back_corona=[]
headers=[]
size_occ=[]
...
for row in range(len(df)):
    ...
    for sat in range(n_sat):
        ...
        if mask_from_cloud:
            mask = maskFromCloud(df.iloc[row].values, sat=sat, satpos=satpos, imsize=imsize, plotranges=plotranges)
        else:
