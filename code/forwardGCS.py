...
OPATH = '/gehme/projects/2020_gcs_with_ml/data/forwardGCS_test'
...
def forwardGCS(configfile_name, headers, size_occ=[2, 3.7, 2]):
    ...
    for row in range(len(df)):
        clouds = getGCS(df['CMElon'][row], df['CMElat'][row], df['CMEtilt']
        ...
        for sat in range(len(satpos)):
            ...
            btot = rtraytracewcs(headers[sat], df['CMElon'][row], df['CMElat'][row],
                                 df['CMEtilt'][row], df['height'][row], df['k'][row], df['ang'][row])
            plt.imshow(btot)
            ...
            fig.savefig(OPATH + '/{:08.3f}_{:08.3f}_{:08.3f}_{:08.3f}_{:08.3f}_{:08.3f}_sat{}.png'.format(
