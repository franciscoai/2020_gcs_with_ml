...
def plot_to_png(ofile,image_path,orig_img, masks,imsize, title=None, labels=None, boxes=None, scores=None):
    ...
    """    
    mask_threshold = 0.8 # value to consider a pixel belongs to the object
    #scr_threshold = 0.6 # only detections with score larger than this value are considered
    color=['r','b','g','k','y']
    ...
    for i in range(len(masks[0])):
        ...
        if labels[0][i]==2:
            ...
            if (pol is not None):
                ...
                # plt.savefig(ofile)
                # plt.close()
                ang_list.append([image_path,i,date_time,scores[0][i], min_ang, max_ang, cpa_ang, wide_ang, cm[1], cm[0], cm_ang, apex_dist, apex_ang])          
...
for folder in folders:
    ...
    fa=fa+1
    columnas = ['PATH',"MASK_LABEL" ,'DATE_TIME', "SCORE",'MIN_ANG','MAX_ANG','CPA_ANG','WIDE_ANG','MASS_CENTER_X','MASS_CENTER_Y','MASS_CENTER_ANG','APEX_RADIUS','APEX_ANG'] 
