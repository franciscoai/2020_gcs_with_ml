...
def get_cdaw(repo, path, opath, exclude_poor_events, exclude_mult):
    '''
    ...
    if exclude_poor_events:
        catalogue['QUALITY_INDEX'] = catalogue['QUALITY_INDEX'].str.extract(r'(\d+)').astype(int)
        catalogue=catalogue.loc[catalogue['QUALITY_INDEX']>quality_idx]
    
    if exclude_mult:
        unique_dates=catalogue["DATE"].unique()
        for j in range(len(unique_dates)):
            date_event=catalogue.loc[catalogue["DATE"]==unique_dates[j]]
            orig_files=date_event['FOLDER_NAME'].unique()
            for g in range(len(orig_files)-1):
                first_event=catalogue.loc[catalogue['FOLDER_NAME']==orig_files[g]]
                second_event=catalogue.loc[catalogue['FOLDER_NAME']==orig_files[g+1]]
                ...
                if first_interval.overlaps(second_interval):
                    catalogue = catalogue.loc[~catalogue['FOLDER_NAME'].str.contains(orig_files[g], case=False, na=False)]
                    catalogue = catalogue.loc[~catalogue['FOLDER_NAME'].str.contains(orig_files[g+1], case=False, na=False)]
...
#gets catalogues
cdaw_full  = get_cdaw(cdaw_repo,lasco_path, cdaw_catalogue,exclude_poor_events,exclude_mult)
