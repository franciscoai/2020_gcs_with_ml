...
OPATH = '/data1/gehme/projects/2023_eeggl_validation/niemela_project/gcs_event_creations/' #'/gehme/projects/2020_gcs_with_ml/data/cme_seg_training' 
...
par_rng = [[-180,180],[-70,70],[-90,90],[2,24],[0.2,0.6], [10,60],[7e2,1e3]] 
    ...
    for sat in range(n_sat):
        ...
        if mask_from_cloud:
            ...
            if mask_npix/cme_npix<0.9:
                ...
                break
        #check for halos
        if np.min(mask[r <= size_occ[sat]*1.1]) ==1:
            halo_count+=1
        ...
        if synth_int_image:          
            ...
            if occ_noise[0] is not None:
                ...
                noise_ext=np.random.normal(loc=occ_noise[0]*level_back, scale=occ_noise[1]*np.abs(level_back), size=imsize)[r >= size_occ_ext[sat]]
            else:
                noise =0        
                noise_ext=0    
