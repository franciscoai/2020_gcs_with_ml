...
model_version="v5"
opath= model_path + "/infer_neural_cme_seg_kincat_"+level+"/"+sat+"_short_version"
file_ext=".fits"
trained_model = '49.torch'
mask_threshold = 0.55 # value to consider a pixel belongs to the object
...
for i in tqdm(range(len(catalogue.index)), desc="Reading Catalogue"):
    ...
    start_date = date_helcat + timedelta(hours=2)
    ...
    if len(files)>0:
        ...
        if len(all_images)>=2:
            ...
            if len(data_list)==3:
                ...
                print("cant de img despues de la red: ",len(ok_dates))
