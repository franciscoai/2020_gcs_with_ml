...
from manage_var_niemela_localpuma import manage_variables_niemela
...
extra_path = "/data1"
#model_path= extra_path+"/gehme/projects/2020_gcs_with_ml/output/neural_cme_seg_v4"
model_path= extra_path+"/gehme/projects/2020_gcs_with_ml/output/neural_cme_seg_v5"
...
device=torch.device('cpu')#cuanto la gpu tiene ram utilizada al 100%
...
occ_size = [50,75] #[50,60] #70 para cor2b
...
for j in range(init_range,len(image_names)):
    ...
    # plot the predicted mask
        ...
        if not infer_event2:
            plot_to_png(ofile, [orig_img], [masks], scores=[scores], labels=[labels], boxes=[boxes],mask_threshold=mask_threshold)
        ...
        if real_img==True:    
            ...
            if instr == 'lascoC2':
                ...
                if int(date_string[:2]) < 94: #soho was launched in 1995
                    ...
                    aux = '19'
                #breakpoint()
                date = datetime.strptime(hdr1['date_obs'],'%Y-%m-%dT%H:%M:%S.%f').replace(microsecond=0).strftime("%Y-%m-%d %H:%M:%S")
                percentiles = [2,98]
                #breakpoint()
