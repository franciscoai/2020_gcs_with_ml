...
def loadData(imgs, batchSize, imageSize=None, file_ext=".png", normalization_func=None, masks2use=None, rnd_rot=False):
    '''
...
"""
#Constants
trainDir = '/gehme/projects/2020_gcs_with_ml/data/cme_seg_20240627'
...
proportion=0.8 #proportion of the dataset to use for training
...
random.shuffle(imgs)
imgs_train = imgs[:int(len(imgs)*proportion)]
imgs_val = imgs[int(len(imgs)*proportion):]
...
for i in range(train_ncases): 
    images, targets= loadData(imgs_train, batchSize, normalization_func=nn_seg.normalize, masks2use=masks2use, rnd_rot=random_rot) # loads a batch of training data
