    ...
    def _filter_masks2(self, dates, masks, scores, labels, boxes, mask_prop,plate_scl,opath,MAX_CPA_DIST,MIN_CPA_DIFF,MIN_CLUSTER_POINTS):
                ...
                for h in range(i + 1, len(clusters_median)):
                    ...
                    if diff < MIN_CPA_DIFF:
                        if optimal_n_clusters==2:
                            ...
                            df['CME_ID'] = 0
                            #el problema esta aca crea un cluster y les pone id 0 falta agregar que se quede solo una for fecha cuando el cluster es 1#
                        else:
        ...
        for k in range(optimal_n_clusters):
            ...
            if (optimal_n_clusters==1) & (len(filtered_df["CME_ID"])<=3):
                ...
                #Fits the corresponding function type and calculates the distance from the masks to the fitted function, the concavity of the function and the median velocity of it   
        ...
        if len(filtered_df["CME_ID"])>3:        
            ...
            if count:
                ...
                selected_cluster["CPA_DIST"] = dist[optimal_vel_idx][0]
                selected_cluster["WA_DIST"] = dist[optimal_vel_idx][1]
                selected_cluster["APEX_DIST"] = dist[optimal_vel_idx][2]
                error=np.sqrt(dist[optimal_vel_idx][0]**2+dist[optimal_vel_idx][1]**2+dist[optimal_vel_idx][2]**2)
                selected_cluster["ERROR"] = error
            ...
            if filter:      
                ...
                if len(df)>0:
                    ...
                    if plot_params is not None:
                        if not os.path.exists(plot_params):
                            os.makedirs(plot_params)
                    ...
                    print("ERROR: Consistent masks not found")
                    return [], [], []   
        print("ERROR: Too short event, less than 3 mask were found")        
        return [], [], []                
