...
def plot_gcs_param_vs_time(fit_par, gcs_manual_par,dates,tiempos, opath,aux='', ylim=None):
    """
    ...
    axs[0].set_ylabel('CME Lon [deg]')
    axs[1].set_ylabel('CME Lat [deg]')
    axs[2].set_ylabel('CME Tilt [deg]')
    axs[3].set_ylabel('Height [Rsun]')
    axs[4].set_ylabel('k')
    axs[5].set_ylabel('Angular Width [deg]')
    plt.tight_layout()
    plt.savefig(os.path.join(opath, f'{aux}gcs_fit_vs_manual.png'))
    plt.close()
...
def gcs_mask_error_diego2(gcs_par):#, satpos, plotranges, masks, imsize, occ_size):
    """
    Computes the error between the input masks and the maks from GCS model
    :param gcs_par: GCS model parameters. The param are: CMElon, CMElat, CMEtilt, k, ang, , height0, height1, height2, ...
                    For all images they are all the same except for height, wich is different for each set of three images (time instant)
    ...
    :return: error
    """
    global satpos, plotranges, meas_masks, imsize, occ_sizes
    occ_size = occ_sizes.copy()
    masks = meas_masks.copy()
    error = []
    #breakpoint()
    #Para cada par o triplete de imagenes, que conrresponden al mismo tiempo, asignarle la misma altura.
    #En caso contrario se ajustan tantas alturas como satpos haya para ese mismo tiempo.
    ...
    for i in range(len(masks)):
        ...
        if i%2==1:
            j=j+1
    return np.sum(np.array(error).flatten())
...
def sensitivity_test(gcs_par_minimum, satpos, plotranges, masks, imsize, occ_size,aux):
    ...
    for i in range(len(gcs_par_minimum)):
        ...
        for j in x:
            ...
            #paralelizar. un proceso por cada parametro.
...
'''
#Constants
dpath =  '/data1/gehme/projects/2023_eeggl_validation/niemela_project/gcs_20100403_mask'
opath = dpath + '/gcs_fit'
...
select=None
manual_gcs_path = '/data1/gehme/projects/2023_eeggl_validation/repo_diego/2020_gcs_with_ml/nn/neural_cme_seg/applications/niemela_proyect/'
...
#set minimizador
least_squares_minimizer  = False
basin_hopping_minimizer  = False
dual_annealing_minimizer = True
evolution_minimizer      = False
...
####
### Crear una funcion gcs_mask_error_diego2 que no requiera como input kwargs. estos deben ser globales.
####
if basin_hopping_minimizer:
    print('Fitting GCS model with Basin Hopping; initial conditions: ', ini_cond_0)
    fit = basinhopping(gcs_mask_error_diego2, ini_cond_0, stepsize=0.5, niter=400)
    # summarize the result
    print('Status : %s' % fit['message'])
    print('Total Evaluations: %d' % fit['nfev'])
    # evaluate solution
    solution = fit['x']
    evaluation = gcs_mask_error_diego2(solution)
    print('Solution: f(%s) = %.5f' % (solution, evaluation))
    aux = 'basin_hopping_minimizer_'
    breakpoint()
    # saves to pickle
    with open(os.path.join(opath, 'gcs_fit_basin_hopping.pkl'), 'wb') as f:
        pickle.dump(fit, f)

if dual_annealing_minimizer:
    # define the bounds on the search
    bounds_annealing=list(zip(low_bounds,up_bounds))
    print('Fitting GCS model with Dual Annealing; Bounds: ', bounds_annealing)
    # perform the dual annealing search
    fit = dual_annealing(gcs_mask_error_diego2, bounds_annealing)
    # summarize the result
    print('Status : %s' % fit['message'])
    print('Total Evaluations: %d' % fit['nfev'])
    # evaluate solution
    ...
    evaluation = gcs_mask_error_diego2(solution)
    print('Solution: f(%s) = %.5f' % (solution, evaluation))
    aux = 'dual_annealing_minimizer_'
    breakpoint()
    # saves to pickle
    with open(os.path.join(opath, 'gcs_fit_dual_annealing.pkl'), 'wb') as f:
        pickle.dump(fit, f)

if evolution_minimizer:
    # define the bounds on the search
    bounds_evolution=list(zip(low_bounds,up_bounds))
    print('Fitting GCS model with Evolution; Bounds: ', bounds_evolution)
    # perform the differential evolution search
    fit = differential_evolution(gcs_mask_error_diego2, bounds_evolution, disp=True)
    # summarize the result
    print('Status : %s' % fit['message'])
    print('Total Evaluations: %d' % fit['nfev'])
    # evaluate solution
    ...
    evaluation = gcs_mask_error_diego2(solution)
    print('Solution: f(%s) = %.5f' % (solution, evaluation))
    aux = 'evolution_minimizer_'
    breakpoint()
    # saves to pickle
    with open(os.path.join(opath, 'gcs_fit_evolution.pkl'), 'wb') as f:
        pickle.dump(fit, f)
...
plot_gcs_param_vs_time(gcs_fit_par, gcs_manual_par, dates,tiempos,opath,aux=aux, ylim=gcs_par_range)
...
for j in range(len(tiempos)):
    ...
    ofile = os.path.join(opath, f'{aux}{cfname}_gcs_fit.png')
...
#aux_name='testeo_sensibilidad_mean'
aux_name='testeo_sensibilidad_sum4'
#aux2=['t0','t1','t2']
breakpoint()
aux_file = aux+aux_name
